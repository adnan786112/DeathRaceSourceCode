...
public class CarAIControl : NetworkBehaviour
{
    ...
    [SerializeField] private float ThresholdTargetAngle = 60;
    ...
    {
        base.OnNetworkSpawn();
        if (AssignPosScript.instance != null && gameObject.GetComponent<WaypointProgressTracker>().enabled == true)
        {
            gameObject.GetComponent<WaypointProgressTracker>().circuit = AssignPosScript.instance.waypointCircuit;
        }
    }
    ...
    private void FixedUpdate()
    {
        ...
        else
        {
            if (SaveScript.RaceStart == true && RaceCompleted == false)
            {
                ...
                if (!BarrierStop && !m_Rigidbody.isKinematic)
                {
    ...
    {
        if (!IsOwner) return;
        if (!CarUserControl.FreezeCarAtStartingLight && playerData.CarLapPosition.Value <= SaveScript.MaxLaps+1)
        {
            m_Rigidbody.isKinematic = false;
            m_Rigidbody.constraints = RigidbodyConstraints.None;
            m_Driving = true;
        }
        if(playerData.CarLapPosition.Value >= SaveScript.MaxLaps+1)
        {
            m_Driving = false;
        }
    ...
    {
        if(other.gameObject.CompareTag("Barrier") && !BarrierStop)
        {
            BarrierStop = true;
    ...
    private void OnCollisionStay(Collision col)
    {
        ...
        if (col.gameObject.layer == 7 && SaveScript.RaceStart)
        {
            StartCoroutine(AITruckBarrier());
        }
    }
    ...
    private IEnumerator AITruckBarrier()
    {
